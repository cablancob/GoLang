			...
			if isDAte(strings.TrimSpace(sheet.Name)) {
				for _, row := range sheet.Rows {
					if strings.TrimSpace(row.Cells[1].String()) != "" && isDAte2(strings.TrimSpace(row.Cells[1].String())) {
						if strings.ToUpper(strings.TrimSpace(row.Cells[6].String())) != "APP" {
							...
							_, err4 := db.Exec("INSERT INTO callcenter.callcenterdata(date_ride, driver, origin, destination, rider, tks) VALUES (to_date('" + strings.TrimSpace(row.Cells[1].String()) + "','YYYY-MM-DD'), '" + strings.TrimSpace(driver) + "', '" + strings.TrimSpace(row.Cells[3].String()) + "', '" + strings.TrimSpace(row.Cells[4].String()) + "', 'a8a90dd8-02fd-4f86-815b-90cab0435d46', '" + strings.TrimSpace(row.Cells[9].String()) + "');")
							if err4 != nil {
								db.Close()
								panic(err4)
							}
